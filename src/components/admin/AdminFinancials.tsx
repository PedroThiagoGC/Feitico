...
export default function AdminFinancials() {
  ...
  const totalDuration = bookings.reduce((a, b) => a + (b.total_duration || 0), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div>
          ...
          <Input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="bg-secondary border-border font-body" />
        </div>
        <div>
          <label className="font-body text-sm">Até</label>
          <Input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="bg-secondary border-border font-body" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          { label: "Tempo Ocupado", value: `${totalOccupied} min`, color: "text-muted-foreground" },
        ].map((c) => (
